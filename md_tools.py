...
def gmx_extract(index, path, rename):
    ...
    Returns:
    Average value of a property with an index <index> from the gmx_mpi energy module.
    """

    legend = ""
    rename_str = rename + ".xvg"
    ...
    os.rename("energy.xvg", rename_str)
    
    index = 0 # number of blank lines in the file
    with open(rename_str, mode='r') as f:
        for line in f:
            if (line.find('#') != -1 or line.find('@') != -1): index = index + 1
            if (line.find("s0 legend") != -1):
                el = line.split('"')
                legend = el[1]
                break
    
    data = np.genfromtxt(rename_str, skip_header=index, dtype=float)
    return np.average(data[:,1]), legend
